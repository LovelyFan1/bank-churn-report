...
print()
print(f"  ★ 净亏者的**平均概率反而更高**（{bp.mean():.4f} > {gp.mean():.4f}）")
print(f"    因为「余额 0」这个特征本身就是流失的强信号 —— 模型判得没错，")
print(f"    但「判得准」不等于「值得救」。")
...
import re
src = open(r"D:\创新创业\创新创业\bank-churn-report\frontend\src\views\CustomerManagement.vue",
           encoding="utf-8").read()
